...
class GameClient:
    ...
    def handle_message(self, message):
        ...
        elif data['type'] == 'get_new_deck':
            self.given_chips = data['new_chips']
        elif data['type'] == 'update_deck':
            self.substitute = data['substitute']
    ...
    def send_get_heap_state_message(self):
        message = pickle.dumps({'type': 'get_heap_state'})
        self.send_message(message)

    def send_exchange_of_chips_message(self, returned_chips):
        message = pickle.dumps({'type': 'exchange_chips', 'returned_chips': returned_chips})
        self.send_message(message)

    def send_give_chips_message(self, n_chips):
        message = pickle.dumps({'type': 'give_chips', 'n_chips': n_chips})
        self.send_message(message)

    def make_an_exchange_of_chip(self) -> None:
        self.send_get_heap_state_message()
        self.check_heap_state()
        if (self.is_heap_empty is not None and not self.is_heap_empty and
    ...
    def make_an_exchange_of_all_unexchanged_chips(self) -> None:
        indexes_of_chips_to_exchange = list(set(self.deck.get_non_empty_places_for_chips_indexes()) - set(self.exchanged_chips_indexes))
        deck_returned_content = copy(self.deck.content_on_places(indexes_of_chips_to_exchange))
        self.send_exchange_of_chips_message(deck_returned_content)
        self.check_substitute()
        if self.substitute is not None:
            n_unexchanged_chips = (len(indexes_of_chips_to_exchange) - len(self.substitute))
            if n_unexchanged_chips:
                indexes_of_chips_to_exchange = indexes_of_chips_to_exchange[:-n_unexchanged_chips]
            self.exchanged_chips_indexes = list(set(self.exchanged_chips_indexes) | set(indexes_of_chips_to_exchange))
            self.deck.place_on_places(self.substitute, indexes_of_chips_to_exchange)
        self.current_chip = None
        self.current_chip_index = -1
    ...
    def complete_the_move(self) -> None:
        ...
        if self.given_chips:
            ...
            if extra_chips:
                ...
                self.new_chips_indexes = list(set(indexes_of_new_chips) - set(self.deck.get_empty_places_for_chips_indexes()))
            self.current_chip = None
            self.current_chip_index = -1

    def check_heap_state(self):
        while self.is_heap_empty is None or self.n_chips_of_heap is None:
            pygame.time.wait(100)
            self.receive_messages()

    def check_substitute(self):
        while self.substitute is None:
            pygame.time.wait(100)
            self.receive_messages()

    def check_give_chips(self):
        while self.given_chips is None:
            pygame.time.wait(100)
            self.receive_messages()
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key in GameConstants.SIMPLE_ACTIONS_KEYS:
                        match event.key:
                            ...
                            case pygame.K_r:
                                if (not self.deck.is_empty() and self.field.has_last_choice_init_value()):
        ...
        for x in range(left_corner_x, right_corner_x, step):
            ...
            if chip is not None:
                ...
                if chip_index == current_chip_index:
                    pygame.draw.rect(screen, GameConstants.PURPLE_COLOR, rect, WindowParameters.CURRENT_CHIP_BORDER_SIZE)
            else:
    ...
    def is_mouse_on_the_field(self, mouse_pos: tuple[int, int]) -> bool:
        x, y = mouse_pos
        return (WindowParameters.DISPLAYED_PART_OF_FIELD_MIN_X <= x <= WindowParameters.DISPLAYED_PART_OF_FIELD_MAX_X) and (WindowParameters.DISPLAYED_PART_OF_FIELD_MIN_Y <= y <= WindowParameters.DISPLAYED_PART_OF_FIELD_MAX_Y)
    ...
    def update_current_chip_parameters(self, certain_value: object, container: list | tuple) -> None:
